...
// Inverse Kinematics
bool Leg::ik(const Eigen::Vector3d &pf, Eigen::Vector3d &thetalist,
             Eigen::Matrix3d *Jinv) {
  /* Inverse Kinematics */
  double x = pf.x();
  double y = pf.y();
...
bool Leg::ivk(const Eigen::Vector3d &vf, const Eigen::Matrix3d &Jinv,
...
bool Leg::id(Eigen::Vector<double, njoints> &taulist) {
  ...
  for (int i = 0; i < njoints; i++) {
    ...
    AdTi[i] =
        ((Ai.col(i) * -thetalist(i)).tose3().exp6() * Mlist[i].inverse()).Ad();
    Vi.col(i + 1) = AdTi[i] * Vi.col(i) + Ai.col(i) * thetadlist(i);
    dVi.col(i + 1) = AdTi[i] * dVi.col(i) + Ai.col(i) * thetaddlist(i) +
                     Vi.col(i + 1).ad() * Ai.col(i) * thetadlist(i);
  }
...
//   switch (state) {
//   case IDLE: {
//     p = home_pos + Eigen::Vector3d(travel * 3 / 5 * legNum - travel, 0,
//     0);
//   } break;
//   case TRAVEL: {
//     dp = v_copy * dt_sec;
...
//   }
//   // If t is in the second half of the cycle, interpolate from lift_pos
//   to
//   // home_pos
//   else {
